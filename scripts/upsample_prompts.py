...
def process_prompt_batch(
    ...
    upsampler = PixtralPromptUpsampler(
        checkpoint_dir=checkpoint_dir, offload_prompt_upsampler=False  # Keep model loaded
    )
        ...
        for i, prompt_data in enumerate(prompts):
            ...
            if preprocess_videos and video_path:
                ...
                processed_video = preprocess_video_for_upsampling(
                    video_path, max_resolution=max_resolution, num_frames=num_frames
                )
            ...
            try:
                ...
                upsampled = upsampler._prompt_upsample(
                    prompt=original_prompt, video_path=processed_video if processed_video else None
                )
...
def main():
    ...
    parser.add_argument(
        "--prompts-file", type=str, required=True, help="JSON file containing prompts to upsample"
    )
    parser.add_argument(
        "--checkpoint-dir",
        type=str,
    ...
    parser.add_argument(
        "--max-resolution", type=int, default=480, help="Maximum resolution for video preprocessing"
    )
    parser.add_argument(
        "--num-frames", type=int, default=2, help="Number of frames to extract from videos"
    )
    parser.add_argument(
        "--output-file",
        type=str,
