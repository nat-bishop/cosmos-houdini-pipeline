...
def main():
    ...
    parser.add_argument(
        "--batch-file", type=str, required=True, help="JSON file with prompts to upsample"
    )
    parser.add_argument(
        "--output-file", type=str, required=True, help="Output JSON file for upsampled prompts"
    )
    parser.add_argument(
        "--checkpoint-dir",
        type=str,
    ...
    parser.add_argument(
        "--num-frames", type=int, default=2, help="Number of frames to extract from video"
    )
    ...
    print("Initializing Pixtral upsampler model...")
    print(f"Checkpoint dir: {args.checkpoint_dir}")

    # Initialize model (loads once unless offloading is enabled)
    upsampler = PixtralPromptUpsampler(
        checkpoint_dir=args.checkpoint_dir, offload_prompt_upsampler=args.offload_model
    )
    ...
    for i, item in enumerate(prompts_batch):
        ...
        # Skip if no prompt
        if not prompt_text:
            print("  Skipping: No prompt text provided")
            results.append(
                {
