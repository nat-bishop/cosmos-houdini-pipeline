...
class UpsampleWorkflowMixin:
    ...
    def run_prompt_upsampling(
        ...
        docker_cmd = [
            "bash", "/home/ubuntu/NatsFS/cosmos-transfer1/scripts/upsample_prompt.sh",
            remote_batch_path,
            remote_output_path,
        ...
        for result in upsampled_results:
            # Find matching spec
            matching_spec = next(
                (s for s in prompt_specs if s.id == result.get("spec_id")),
                None
            )
    ...
    def run_single_prompt_upsampling(
        self,
    ...
    def run_prompt_upsampling_from_directory(
        ...
        **kwargs
        ...
        if not prompts_path.exists():
            ...
            return {
                "success": False,
                "error": f"Directory not found: {prompts_dir}"
            }
        ...
        if not prompt_specs:
            ...
            return {
                "success": True,
                "num_upsampled": 0,
                "message": "No prompts needed upsampling"
            }
