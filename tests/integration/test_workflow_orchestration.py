...
class TestWorkflowOrchestration:
    ...
    @pytest.fixture
    def workflow_orchestrator(self, mock_config_manager, mock_ssh_manager, 
                            mock_file_transfer, mock_docker_executor):
        """Create WorkflowOrchestrator with mocked dependencies."""
        with patch('cosmos_workflow.workflows.workflow_orchestrator.SSHManager') as mock_ssh_class, \
             patch('cosmos_workflow.workflows.workflow_orchestrator.FileTransferManager') as mock_ft_class, \
             patch('cosmos_workflow.workflows.workflow_orchestrator.DockerExecutor') as mock_docker_class:
    ...
    def test_complete_inference_workflow(self, workflow_orchestrator, sample_run_spec,
                                        sample_prompt_spec, temp_dir):
        ...
        with patch('cosmos_workflow.prompts.prompt_spec_manager.PromptSpecManager.load_by_id') as mock_load:
    ...
    def test_video_directory_detection(self, workflow_orchestrator, temp_dir):
        ...
        with patch.object(workflow_orchestrator, 'config_manager') as mock_config:
            ...
            video_dirs = workflow_orchestrator._get_video_directories(
                "scene1", 
                str(run_spec_file)
            )
    ...
    @pytest.mark.integration
    def test_control_spec_generation(self, workflow_orchestrator, sample_prompt_spec,
                                    sample_run_spec, temp_dir):
    ...
    @pytest.mark.integration
    def test_error_recovery_workflow(self, workflow_orchestrator, sample_run_spec,
                                    sample_prompt_spec, temp_dir):
    ...
    def test_parallel_upload_optimization(self, workflow_orchestrator, temp_dir):
        ...
        for video_file in video_files:
            workflow_orchestrator.file_transfer.upload_file(
                str(video_file),
                f"/remote/videos/{video_file.name}"
            )
    ...
    @pytest.mark.integration
    def test_prompt_upsampling_integration(self, workflow_orchestrator, 
                                          sample_prompt_spec, temp_dir):
        ...
        with patch.object(workflow_orchestrator, 'run_prompt_upsampling') as mock_upsample:
            ...
            result = mock_upsample(
                str(prompt_file),
                video_path=sample_prompt_spec.input_video_path
            )
