...
class SchemaUtils:
    ...
    def validate_parameters(parameters: dict[str, Any]) -> bool:
        ...
        try:
            if not isinstance(parameters["num_steps"], int) or not (
                1 <= parameters["num_steps"] <= 100
            ):
                return False
            ...
            if not isinstance(parameters["blur_strength"], str) or parameters[
                "blur_strength"
            ] not in [e.value for e in BlurStrength]:
                return False

            if not isinstance(parameters["canny_threshold"], str) or parameters[
                "canny_threshold"
            ] not in [e.value for e in CannyThreshold]:
                return False
