...
class TestEndToEndPipeline:
    ...
    def create_test_frames(self, temp_dir):
        ...
        def _create_frames(base_dir, modalities=['color', 'depth', 'segmentation'], 
                          frame_count=10):
    ...
    @pytest.mark.system
    @pytest.mark.slow
    def test_complete_pipeline_from_frames_to_video(self, mock_environment, 
                                                   create_test_frames, temp_dir):
        ...
        with patch('cosmos_workflow.cli.VideoProcessor') as mock_processor, \
    ...
    def test_pipeline_with_ai_description(self, mock_environment, create_test_frames):
        ...
        with patch('cosmos_workflow.cli.VideoMetadataExtractor') as mock_extractor, \
    ...
    def test_batch_processing_pipeline(self, mock_environment, create_test_frames):
        ...
        for scene in scenes:
            render_dir = create_test_frames(
                mock_environment['renders'] / scene,
                modalities=['color', 'depth'],
                frame_count=24
            )
    ...
    def test_error_recovery_and_retry(self, mock_environment, create_test_frames):
        ...
        render_dir = create_test_frames(
            mock_environment['renders'] / 'test_recovery',
            modalities=['color'],
            frame_count=10
        )
    ...
    def test_resource_cleanup(self, mock_environment, create_test_frames):
        ...
        render_dir = create_test_frames(
            mock_environment['renders'] / 'cleanup_test',
            modalities=['color'],
            frame_count=5
        )
