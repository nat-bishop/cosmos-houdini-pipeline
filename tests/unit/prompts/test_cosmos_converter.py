...
from cosmos_workflow.prompts import CosmosConverter, PromptSpec, RunSpec
...
class TestCosmosConverter:
    ...
    def setup_method(self):
        ...
        self.run_spec = RunSpec(
            ...
            name="test_run",
            control_weights={"vis": 0.3, "edge": 0.4, "depth": 0.2, "seg": 0.1},
            parameters={"num_steps": 35, "guidance": 8.0, "sigma_max": 70.0, "seed": 42, "fps": 24},
            timestamp="2025-08-30T12:00:00Z",
    ...
    def test_validate_cosmos_spec_invalid(self):
        ...
        # Missing prompt
        invalid_spec1 = {"input_video_path": "test.mp4", "depth": {"control_weight": 0.5}}
        assert converter.validate_cosmos_spec(invalid_spec1) is False

        # Missing input_video_path
        invalid_spec2 = {"prompt": "Test prompt", "depth": {"control_weight": 0.5}}
        ...
        invalid_spec4 = {
            "prompt": "Test prompt",
            "input_video_path": "test.mp4",
            "depth": {"input_control": "depth.mp4"},  # Missing control_weight
        }
    ...
    def test_create_upscaler_spec(self):
        ...
        converter = CosmosConverter()
        upscaler_spec = converter.create_upscaler_spec("input_video.mp4", upscale_weight=0.7)
    ...
    def test_merge_specs(self):
        ...
        base_spec = {
            ...
            "depth": {"control_weight": 0.5, "input_control": "depth.mp4"},
        }

        override_spec = {
            "prompt": "Override prompt",  # Override
            "depth": {"control_weight": 0.8},  # Partial override
            "edge": {"control_weight": 0.3},  # New modality
        }
