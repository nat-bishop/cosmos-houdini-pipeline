...
def create_prompt_specs_cli(prompts, video_path=None, output_dir="inputs/prompts"):
    ...
    for prompt in prompts:
        ...
        # Build command
        cmd = ["python", "-m", "cosmos_workflow.cli", "create-spec", name, text]
    ...
    for res in resolutions:
        if res in video_paths and video_paths[res].exists():
            ...
            res_specs = create_prompt_specs_cli(
                prompts, video_path=video_paths[res], output_dir=f"inputs/prompts_{res}"
            )
...
def main():
    ...
    if not test_video_dir.exists():
        ...
        specs = create_prompt_spec_batch(
            TEST_PROMPTS, video_paths=available_videos, resolutions=list(available_videos.keys())
        )
    ...
    print(
        "   python -m cosmos_workflow.cli upsample inputs/prompts/[date]/[name]_ps_*.json --verbose"
    )
    ...
    print(
        "   python -m cosmos_workflow.cli upsample inputs/prompts_720p/ --save-dir outputs/upsampled_720p"
    )
    print("\n4. Test vocab error threshold:")
    print(
        "   python -m cosmos_workflow.cli upsample inputs/prompts_1080p/ --preprocess-videos false"
    )
