...
class TestSmartNamingUtility:
    ...
    def test_generate_smart_name_from_prompts(self):
        ...
        test_cases = [
            ("Futuristic cyberpunk city with neon lights", ["futuristic", "cyberpunk", "city", "neon", "lights"]),
            ("A beautiful sunset over the ocean waves", ["beautiful", "sunset", "ocean", "waves"]),
            ("Transform this into an anime style painting", ["transform", "anime", "style", "painting"]),
            ("Make it look like a Van Gogh masterpiece", ["make", "look", "van", "gogh", "masterpiece"]),
            ("Abstract geometric patterns with vibrant colors", ["abstract", "geometric", "patterns", "vibrant", "colors"]),
        ]
        ...
        for prompt, expected_words in test_cases:
            ...
            matches = sum(1 for word in expected_words if any(word in part.lower() for part in name_parts))
            assert matches >= 1, \
...
class TestPromptSpecSmartNaming:
    ...
    def test_prompt_spec_with_auto_name(self, dir_manager):
        ...
        assert spec.name != ""
        assert "futuristic" in spec.name.lower() or "cityscape" in spec.name.lower() or "flying" in spec.name.lower()
    ...
    def test_prompt_spec_paths_with_auto_name(self, dir_manager):
        ...
        spec = manager.create_prompt_spec(
            prompt_text="Transform into cyberpunk style"
        )
    ...
    def test_upsampled_prompt_with_auto_name(self, dir_manager):
        ...
        upsampled_prompt = "A highly detailed photorealistic scene with dramatic lighting and intricate textures"
        
        spec = manager.create_prompt_spec(
            prompt_text=upsampled_prompt,
            is_upsampled=True,
            parent_prompt_text=original_prompt
        )
        
        # Verify name was generated from upsampled prompt
        assert spec.name is not None
        assert any(word in spec.name.lower() for word in ["detailed", "photorealistic", "dramatic", "lighting"])
...
"""
    ...
    def test_prompt_manager_auto_naming(self, temp_config, tmp_path):
        ...
        with patch('cosmos_workflow.config.config_manager.ConfigManager') as mock_config:
            mock_config.return_value.get_local_config.return_value.prompts_dir = str(temp_config.parent / "prompts")
            mock_config.return_value.get_local_config.return_value.runs_dir = str(temp_config.parent / "runs")
            ...
            assert spec.name is not None
            assert any(word in spec.name.lower() for word in ["underwater", "coral", "tropical", "fish", "scene"])
...
class TestCLISmartNaming:
    ...
    def test_create_spec_command_without_name(self):
        ...
        with patch('cosmos_workflow.config.config_manager.ConfigManager'), \
             patch('cosmos_workflow.prompts.schemas.DirectoryManager') as mock_dir_manager, \
             patch('cosmos_workflow.prompts.schemas.PromptSpec.save'):
