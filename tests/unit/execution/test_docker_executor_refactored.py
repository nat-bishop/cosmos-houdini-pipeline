...
class TestDockerExecutorIntegration:
    ...
    def test_full_inference_pipeline(self, tmp_path):
        ...
        assert any("mountain_scene" in cmd for cmd in mkdir_commands)

        # Verify inference completed
        assert "mountain_scene" in executor.inference_results
    ...
    def test_concurrent_inference_isolation(self, tmp_path):
        ...
        for i, prompt in enumerate(prompts):
            name = f"scene_{i}"
            assert name in executor.inference_results
            result = executor.inference_results[name]
            assert f"scene_{i}" in result["output_path"]
