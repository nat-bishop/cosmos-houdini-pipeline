...
cat > "outputs/${PROMPT_NAME}/spec_used.json" <<JSON
{
  "prompt_spec": $(cat "inputs/prompts/${PROMPT_NAME}.json"),
  "inference_command": "torchrun --nproc_per_node=\$NUM_GPU --nnodes=1 --node_rank=0 cosmos_transfer1/diffusion/inference/transfer.py --checkpoint_dir \$CHECKPOINT_DIR --video_save_folder outputs/${PROMPT_NAME} --controlnet_specs inputs/prompts/${PROMPT_NAME}.json --offload_text_encoder_model --upsample_prompt --offload_prompt_upsampler --offload_guardrail_models --num_gpus \$NUM_GPU",
  "environment": {
