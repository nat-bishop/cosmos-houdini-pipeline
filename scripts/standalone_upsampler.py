...
def main():
    ...
    parser.add_argument("--batch-file", type=str, help="JSON file with prompts to upsample")
    parser.add_argument(
        "--prompt", type=str, help="Single prompt to upsample (alternative to batch file)"
    )
    ...
    parser.add_argument(
        "--output-file",
        type=str,
        default="/workspace/outputs/upsampled_result.json",
        help="Output file for upsampled result",
    )
    parser.add_argument(
        "--offload", action="store_true", help="Offload model after inference to save memory"
    )
    ...
    upsampler = PixtralPromptUpsampler(
        checkpoint_dir=args.checkpoint_dir, offload_prompt_upsampler=args.offload
    )
