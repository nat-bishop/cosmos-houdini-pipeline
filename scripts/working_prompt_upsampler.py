...
try:
    ...
    print("Make sure you're in the cosmos-transfer1 directory or have it in PYTHONPATH", file=sys.stderr)
    sys.exit(1)
...
def upsample_prompt(
    ...
    """
    print(f"[INFO] Initializing Pixtral upsampler...", flush=True)
    ...
    upsampler = PixtralPromptUpsampler(
        ...
        offload_prompt_upsampler=offload
    )
...
def process_batch(
    ...
    upsampler = None
    if not offload:
        print(f"[INFO] Loading Pixtral model (keeping in memory)...", flush=True)
        upsampler = PixtralPromptUpsampler(
            checkpoint_dir=checkpoint_dir,
        ...
        try:
            if offload:
                ...
                upsampled = upsample_prompt(
                    prompt, video_path, checkpoint_dir, offload=True
                )
            else:
...
def main():
    ...
    parser.add_argument(
        "--prompt", 
        help="Text prompt to upsample (for single prompt mode)"
    )
    parser.add_argument(
        "--video", 
        help="Video path for visual context (for single prompt mode)"
    )
    parser.add_argument(
        "--batch", 
        help="JSON file with batch of prompts to process"
    )
    parser.add_argument(
        "--output-dir", 
        default="outputs/upsampled",
    ...
    if args.batch:
        ...
        print("[ERROR] Provide either --batch for batch processing or --prompt and --video for single prompt", file=sys.stderr)
        parser.print_help()
        sys.exit(1)
