...
class TestAIIntegration:
    ...
    def test_ai_description_generation_real(self):
        ...
        for img_type, img_path in self.test_images.items():
            ...
            assert word_count >= 3, \
                f"Description too short for {img_type}: {description}"
        ...
        for img_type, img_path in self.test_images.items():
            ...
            if description and "Sequence with" not in description:
                ...
                for part in name_parts:
                    assert part not in stop_words, \
                        f"Name contains stop word '{part}': {name}"
    ...
    def test_performance_with_multiple_frames(self):
        ...
        assert multi_time < single_time * 2, \
            "Processing multiple frames too slow"
...
class TestSmartNameAlgorithm:
    ...
    def test_stop_word_removal(self):
        ...
        test_cases = [
            ...
            ("the big red house with a garden", ["big", "red", "house"]),  # garden might not make it in top 3
            ("an interesting painting of a landscape", ["painting", "landscape"]),  # interesting has -ing suffix
        ]
...
class TestDirectoryNamingCompliance:
    ...
    def test_timestamp_format_compliance(self):
        ...
        for dt in test_dates:
            ...
            assert pattern.match(timestamp), \
                f"Timestamp '{timestamp}' doesn't match format"
    ...
    def test_full_directory_name_format(self):
        ...
        for name in test_names:
            ...
            assert pattern.match(full_name), \
                f"Directory name '{full_name}' doesn't match format"
