    ...
    try:
        ...
        try:
            result = ssh.execute_command_success(
                'cat /home/ubuntu/NatsFS/cosmos-transfer1/outputs/standalone_test.json',
                timeout=10
            )
            print(result)
        except:
        ...
        containers = ssh.execute_command_success('sudo docker ps --format "table {{.ID}}\t{{.Status}}\t{{.Names}}"', timeout=5)
